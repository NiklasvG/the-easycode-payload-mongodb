...
export default async function ProjectDetailPage({ params: paramsPromise }: Args) {
  ...
  const primaryContact =
    clientObj?.contacts?.find((contact) => contact.comment && contact.comment.trim().length > 0) ??
    null
            ...
            <div className="grid md:grid-cols-2 gap-12">
              <div>
                ...
                <div className="text-gray-300 leading-relaxed text-lg">
                  {project.theChallenge && <RichText data={project.theChallenge} />}
                </div>
              </div>
              <div>
                <h3 className="text-2xl font-display font-bold mb-4 text-white flex items-center gap-2">
            ...
            {project.secondaryImages && project.secondaryImages.length > 0 && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 my-12">
                {project.secondaryImages.map((imgItem, idx) => {
                  const img = typeof imgItem.image === 'object' ? imgItem.image : null
              ...
              <div className="bg-gradient-to-br from-secondary-background to-background border border-white/10 p-8 rounded-3xl relative">
                ...
                <blockquote className="relative z-10">
                  ...
                  <footer className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-bold text-white text-sm">{quote.author}</div>
                      {quote.role && <div className="text-xs text-accent">{quote.role}</div>}
                    </div>
                  </footer>
                </blockquote>
              </div>
            ...
            {project.theNerdDetails && (
              <div className="bg-[#0f0f0f] border border-gray-800 rounded-2xl p-6 relative overflow-hidden group">
                ...
                <div className="text-gray-400 text-sm leading-relaxed font-mono">
                  {project.theNerdDetails && <RichText data={project.theNerdDetails} />}
                </div>
              </div>
            )}
